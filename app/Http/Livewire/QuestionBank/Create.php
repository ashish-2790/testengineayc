...
class Create extends Component
{
    ...
    public function mount(QuestionBank $questionBank)
    {
        ...
        $this->questionBank->example_question = false;
        $this->questionBank->inactive         = false;
        $this->initListsForFields();

        $questpaper_id = $_GET['qid']??'';
        $ability_id = $_GET['aid']??'';

        $this->questionBank->related_question_paper_id = $questpaper_id??'';

        $get_question_paper = QuestionPaper::where('id',$questpaper_id)->first();

        $this->questionBank->related_test_type_id = $get_question_paper->related_test_id??'';

        $this->questionBank->related_ability_id = $ability_id??'';
